...
impl Bytes {
  pub fn qty_to_bytes(s: &str) -> Option<f64> {
    ...
    for c in s.chars() {
      if c.is_ascii_digit() || c == '.' { num_part.push(c); } else { unit_part.push(c); }
    }
  ...
  pub fn format_bytes_decimal(bytes: f64) -> String {
    ...
    if bytes >= TB { return format!("{:.2} TB", bytes / TB); }
    if bytes >= GB { return format!("{:.2} GB", bytes / GB); }
    if bytes >= MB { return format!("{:.2} MB", bytes / MB); }
    if bytes >= KB { return format!("{:.2} KB", bytes / KB); }
    format!("{:.0} B", bytes)
  }

  pub fn pretty_size(q: &str) -> String {
    match Self::qty_to_bytes(q) { Some(b) => Self::format_bytes_decimal(b), None => q.to_string() }
  }
}