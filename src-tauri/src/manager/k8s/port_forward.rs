...
impl<'a> PortForwarder<'a> {
  ...
  async fn resolve_target_pod(
    ...
    match resource_kind.as_str() {
      ...
      "service" | "svc" => {
        let svc_api: Api<Service> = K8sClient::api::<Service>(client.clone(), Some(namespace.clone())).await;
        ...
        let pods_api: Api<Pod> = K8sClient::api::<Pod>(client.clone(), Some(namespace.clone())).await;
        ...
        let pods = pods_api
          .list(&lp)
          .await
          .map_err(|e| format!("failed to list pods for service {}: {}", resource_name, e))?;
        let name = pods
          .items
          .into_iter()
      ...
      "deployment" | "deploy" => {
        let api: Api<Deployment> = K8sClient::api::<Deployment>(client.clone(), Some(namespace.clone())).await;
        ...
        let pods = pods_api
          .list(&lp)
          .await
          .map_err(|e| format!("failed to list pods for deployment {}: {}", resource_name, e))?;
        let name = pods
          .items
          .into_iter()
          .find_map(|p| p.metadata.name)
      ...
      "replicaset" | "rs" => {
        let api: Api<ReplicaSet> = K8sClient::api::<ReplicaSet>(client.clone(), Some(namespace.clone())).await;
        ...
        let pods = pods_api
          .list(&lp)
          .await
          .map_err(|e| format!("failed to list pods for replicaset {}: {}", resource_name, e))?;
        let name = pods
          .items
          .into_iter()
          .find_map(|p| p.metadata.name)
      ...
      "statefulset" | "sts" => {
        let api: Api<StatefulSet> = K8sClient::api::<StatefulSet>(client.clone(), Some(namespace.clone())).await;
        ...
        let pods = pods_api
          .list(&lp)
          .await
          .map_err(|e| format!("failed to list pods for statefulset {}: {}", resource_name, e))?;
        let name = pods
          .items
          .into_iter()
          .find_map(|p| p.metadata.name)
      ...
      "daemonset" | "ds" => {
        let api: Api<DaemonSet> = K8sClient::api::<DaemonSet>(client.clone(), Some(namespace.clone())).await;
        ...
        let pods = pods_api
          .list(&lp)
          .await
          .map_err(|e| format!("failed to list pods for daemonset {}: {}", resource_name, e))?;
        let name = pods
          .items
          .into_iter()
          .find_map(|p| p.metadata.name)
      ...
      match pods_api.portforward(&pod_name, &[remote_port]).await {
        Ok(mut pf) => {
          match TcpListener::bind(("127.0.0.1", local_port)).await {
            Ok(listener) => {
              let _ = app.emit(&ev_name, PortForwardEventPayload { r#type: "PF_STDOUT".into(), line: format!("listening on 127.0.0.1:{} -> {}:{}", local_port, pod_name, remote_port) });
              loop {
                tokio::select! {
                  _ = kill_rx.recv() => {
            ...
            Err(e) => {
              let _ = app.emit(&ev_name, PortForwardEventPayload { r#type: "PF_ERROR".into(), line: format!("failed to bind local port {}: {}", local_port, e) });
            }
          }
        }
        Err(e) => {
          let _ = app.emit(&ev_name, PortForwardEventPayload { r#type: "PF_ERROR".into(), line: format!("portforward start error: {}", e) });
        }
      }
    });
  ...
  pub async fn stop(state: tauri::State<'_, PortForwardManager>, session_id: String) -> Result<(), String> {
