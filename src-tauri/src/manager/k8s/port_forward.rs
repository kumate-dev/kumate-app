...
use kube::Client;
// rand 0.10 shuffled the trait names: the core trait is now `Rng` (was `RngCore`) and the
// sampling helpers `random`/`random_range` (was `gen`/`gen_range`) live on `RngExt`.
use rand::RngExt;
use serde::Serialize;
    ...
    async fn resolve_target_pod(
        ...
        match resource_kind.as_str() {
            ...
            "replicaset" | "rs" => {
                ...
                let pods = pods_api.list(&lp).await.map_err(|e| {
                    format!(
                        "failed to list pods for replicaset {}: {}",
                        resource_name, e
                    )
                })?;
                let name = pods
            ...
            "statefulset" | "sts" => {
                ...
                let pods = pods_api.list(&lp).await.map_err(|e| {
                    format!(
                        "failed to list pods for statefulset {}: {}",
                        resource_name, e
                    )
                })?;
                let name = pods
