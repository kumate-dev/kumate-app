...
///
/// The backend previously used bare `println!`/`eprintln!`, which on a bundled
/// macOS `.app` lands in the system log with no level filtering and no way to turn
/// it off — and at least two of those calls printed credential-adjacent material.
/// Everything now goes through `tracing`, and messages derived from errors are run
/// through [`error::redact`] first.
///
/// Verbosity is controlled by `KUMATE_LOG` (same syntax as `RUST_LOG`), defaulting
/// to `info` for our own crate and `warn` for dependencies.
fn init_tracing() {
    use tracing_subscriber::{fmt, prelude::*, EnvFilter};

    let filter = EnvFilter::try_from_env("KUMATE_LOG")
        .unwrap_or_else(|_| EnvFilter::new("warn,kumate_lib=info"));

    // `try_init` rather than `init`: a second call (tests, mobile entry point)
    // must not panic.
...
/// Remove anything left in the private scratch directory by a previous run.
///
/// Decrypted kubeconfigs and Helm values files live there. They are deleted
/// immediately after use and on the error path, but a hard crash or SIGKILL can
/// still leave one behind, so we sweep on startup.
fn purge_scratch_dir() {
    let dir = &*crate::constants::app::APP_TMP_DIR;
    if !dir.exists() {
        return;
    }
    match std::fs::read_dir(dir) {
        ...
        Err(err) => tracing::warn!(error = %err, "could not purge scratch directory"),
    }
}
...
pub fn run() {
    ...
    builder = builder.setup(|app| {
        ...
        // Propagate the failure instead of `expect("init db")`. A stale sled lock —
        // which happens routinely if a previous process was killed — used to abort
        // the process with a raw panic message and no way for the user to tell what
        // had gone wrong.
        let state = tauri::async_runtime::block_on(state::AppState::init(data_dir))
            .map_err(|e| format!("failed to open the application database: {e}"))?;
    ...
    builder = builder
        ...
        // NOTE: no `.manage(ConnectionsManager)`. It used to be managed here *and*
        // exist as a `OnceLock` global; every reader used the global, so the managed
        // copy was dead weight that could silently drift. There is now one instance,
        // reachable via `ConnectionsManager::global()`.
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            common::unwatch,
            common::unwatch_cluster,
            common::watchers_count,
            common::snapshot_resources,
            common::watch_diagnostics,
            events::list_events,
            nodes::list_nodes,
            nodes::watch_nodes,
            nodes::delete_nodes,
            ...
            config_maps::patch_config_map,
            secrets::create_secret,
            secrets::update_secret,
            secrets::list_secrets,
            secrets::watch_secrets,
            secrets::delete_secrets,
            secrets::patch_secret,
            resource_quotas::create_resource_quota,
            resource_quotas::update_resource_quota,
            resource_quotas::list_resource_quotas,
            resource_quotas::watch_resource_quotas,
            resource_quotas::delete_resource_quotas,
            limit_ranges::create_limit_range,
            limit_ranges::update_limit_range,
            limit_ranges::list_limit_ranges,
            limit_ranges::watch_limit_ranges,
            limit_ranges::delete_limit_ranges,
            horizontal_pod_autoscalers::create_horizontal_pod_autoscaler,
            horizontal_pod_autoscalers::update_horizontal_pod_autoscaler,
            ...
            pod_disruption_budgets::create_pod_disruption_budget,
            pod_disruption_budgets::update_pod_disruption_budget,
            ...
            helm::helm_get_values,
            helm::helm_get_history,
            helm::helm_upgrade_release,
            helm::helm_rollback_release,
    ...
    // Report startup failure rather than panicking: a panic here surfaces as a
    // silent crash with no window and a backtrace the user never sees.
    if let Err(err) = builder.run(tauri::generate_context!()) {
        tracing::error!(error = %err, "fatal: could not start Kumate");
        std::process::exit(1);
    }
}
