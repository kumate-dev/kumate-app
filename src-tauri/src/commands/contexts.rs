...
use crate::{
    k8s::contexts::{
        K8sContexts, KCNamedCluster, KCNamedContext, KCNamedUser, KubeConfigOut, KubeConfigRaw,
        OutNamedCluster, OutNamedContext, OutNamedUser,
    },
    state::AppState,
    utils::crypto::{self, Crypto},
};
...
pub async fn add_context(
    ...
    sqlx::query(r#"INSERT INTO contexts(name, cluster, user, namespace) VALUES(?, ?, ?, ?)"#)
        .bind(&payload.name)
...
pub async fn import_kube_contexts(state: tauri::State<'_, AppState>) -> Result<usize, String> {
    ...
    let mut rd: tokio::fs::ReadDir = match tokio::fs::read_dir(&kube_dir).await {
        Ok(r) => r,
        Err(e) => {
            println!("import_kube_contexts: read_dir error: {}", e);
            return Ok(0);
        }
    };
    ...
    while let Some(entry) = rd.next_entry().await.map_err(|e| e.to_string())? {
        ...
        let raw: KubeConfigRaw = match serde_yaml::from_str(&content) {
            Ok(v) => v,
            Err(e) => {
                println!("serde_yaml parse error: {}", e);
                continue;
            }
        };
        ...
        for nc in contexts.iter() {
            ...
            let cluster_entry: Option<KCNamedCluster> = clusters
                .iter()
                .find(|c: &&KCNamedCluster| c.name == nc.context.cluster)
                .cloned();
            let user_entry: Option<KCNamedUser> =
                nc.context.user.as_ref().and_then(|uname: &String| {
                    users
                        .iter()
                        .find(|u: &&KCNamedUser| u.name == *uname)
                        .cloned()
                });
            ...
            let yaml: String =
                serde_yaml::to_string(&out).map_err(|e: serde_yaml::Error| e.to_string())?;
            ...
            sqlx::query(
                "INSERT OR IGNORE INTO contexts(name, cluster, user, namespace) VALUES(?, ?, ?, ?)",
            )
            .bind(&name)
