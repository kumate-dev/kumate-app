...
pub async fn helm_list_releases(
    ...
    let use_all = namespaces
        .as_ref()
        .map(|ns| ns.is_empty() || ns.iter().any(|n| {
...
fn build_release_from_secret(s: &Secret) -> Value {
    ...
    let revision = labels.get("version").cloned().unwrap_or_else(|| {
        if let Some(idx) = name_field.rfind(".v") { name_field[idx + 2..].to_string() } else { String::new() }
    });
    let status = labels.get("status").cloned().unwrap_or_default();
    let namespace = s.metadata.namespace.clone().unwrap_or_default();
...
async fn watch_stream_secrets(
    ...
    let wp: WatchParams = WatchParams { send_initial_events: true, ..Default::default() };
                    ...
                    match status {
                        Ok(WatchEvent::Added(obj)) => {
                            if is_helm_secret(&obj) {
                                emit_helm_event(&app_handle_clone, &event_name_clone, EventType::ADDED, build_release_from_secret(&obj));
                            }
                        }
                        Ok(WatchEvent::Modified(obj)) => {
                            if is_helm_secret(&obj) {
                                emit_helm_event(&app_handle_clone, &event_name_clone, EventType::MODIFIED, build_release_from_secret(&obj));
                            }
                        }
                        Ok(WatchEvent::Deleted(obj)) => {
                            if is_helm_secret(&obj) {
                                emit_helm_event(&app_handle_clone, &event_name_clone, EventType::DELETED, build_release_from_secret(&obj));
                            }
                        }
                    ...
                    match status {
                        Ok(WatchEvent::Added(obj)) => {
                            if is_helm_config_map(&obj) {
                                emit_helm_event(&app_handle_clone, &event_name_clone, EventType::ADDED, build_release_from_config_map(&obj));
                            }
                        }
                        Ok(WatchEvent::Modified(obj)) => {
                            if is_helm_config_map(&obj) {
                                emit_helm_event(&app_handle_clone, &event_name_clone, EventType::MODIFIED, build_release_from_config_map(&obj));
                            }
                        }
                        Ok(WatchEvent::Deleted(obj)) => {
                            if is_helm_config_map(&obj) {
                                emit_helm_event(&app_handle_clone, &event_name_clone, EventType::DELETED, build_release_from_config_map(&obj));
                            }
                        }
...
pub async fn watch_helm_releases(
    ...
    let watch_fn = Arc::new(|app_handle: AppHandle, name: String, namespaces: Option<Vec<String>>, event_name: String| async move {
