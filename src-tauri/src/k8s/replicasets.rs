use k8s_openapi::{api::apps::v1::{ReplicaSet, ReplicaSetSpec, ReplicaSetStatus}, apimachinery::pkg::apis::meta::v1::Time};
use kube::{api::{ListParams, ObjectList}, Api, Client, ResourceExt};
use serde::Serialize;
...
impl K8sReplicaSets {
    pub async fn list(name: String, namespace: Option<String>) -> Result<Vec<ReplicaSetItem>, String> {
    ...
    async fn fetch(client: Client, namespace: Option<String>) -> Result<Vec<ReplicaSet>, String> {
        ...
        let list: ObjectList<ReplicaSet> = api.list(&lp).await.map_err(|e: kube::Error| e.to_string())?;
        Ok(list.items)
    }

    fn to_item(r: ReplicaSet) -> ReplicaSetItem {
        let replicas: i32 = r.spec.as_ref().and_then(|sp: &ReplicaSetSpec| sp.replicas).unwrap_or(0);
        let ready: i32 = r.status.as_ref().and_then(|st: &ReplicaSetStatus| st.ready_replicas).unwrap_or(0);
        ReplicaSetItem {
            name: r.name_any(),
