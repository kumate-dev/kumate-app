...
pub struct KCNamedContext { pub name: String, pub context: KCContextRef }
#[derive(Deserialize, Serialize, Clone)]
pub struct KCContextRef { pub cluster: String, pub user: Option<String>, pub namespace: Option<String> }
#[derive(Deserialize, Clone)]
pub struct KCNamedCluster { pub name: String, pub cluster: serde_yaml::Value }
#[derive(Deserialize, Clone)]
pub struct KCNamedUser { pub name: String, pub user: serde_yaml::Value }
#[derive(Deserialize, Clone)]
...
#[derive(Serialize)]
pub struct OutNamedContext { pub name: String, pub context: KCContextRef }
#[derive(Serialize)]
pub struct OutNamedCluster { pub name: String, pub cluster: serde_yaml::Value }
#[derive(Serialize)]
pub struct OutNamedUser { pub name: String, pub user: serde_yaml::Value }
#[derive(Serialize)]
...
impl K8sContexts {
  ...
  fn try_read_secrets(
    ...
    match (crypto::secrets_get(kc_key), crypto::secrets_get(token_key)) {
      (Ok(kc_b64), Ok(token_b64)) => {
        let kc_ct: Vec<u8> = STANDARD.decode(kc_b64).map_err(|e: DecodeError| e.to_string())?;
        let token_ct: Vec<u8> = STANDARD.decode(token_b64).map_err(|e: DecodeError| e.to_string())?;

        let kc_pt: Vec<u8> = crypto.decrypt(&kc_ct).map_err(|e: anyhow::Error| e.to_string())?;
        let token_pt: Vec<u8> = crypto.decrypt(&token_ct).map_err(|e: anyhow::Error| e.to_string())?;

        let kc: String = String::from_utf8(kc_pt).map_err(|e: FromUtf8Error| e.to_string())?;
        let token: String = String::from_utf8(token_pt).map_err(|e: FromUtf8Error| e.to_string())?;
        Ok(Some((kc, token)))
      }
      _ => Ok(None),
    }
  ...
  async fn build_kubeconfig_from_home(name: &str) -> Result<String, String> {
    ...
    while let Some(entry) = rd.next_entry().await.map_err(|e: std::io::Error| e.to_string())? {
      let ft: FileType = entry.file_type().await.map_err(|e: std::io::Error| e.to_string())?;
      ...
      let content: String = match tokio::fs::read_to_string(&path).await { Ok(s) => s, Err(_) => continue };
      let raw: KubeConfigRaw = match serde_yaml::from_str(&content) { Ok(v) => v, Err(_) => continue };

      let contexts: Vec<KCNamedContext> = match raw.contexts { Some(v) => v, None => continue };
      let clusters: Vec<KCNamedCluster> = raw.clusters.unwrap_or_default();
      let users: Vec<KCNamedUser> = raw.users.unwrap_or_default();

      if let Some(yaml) = Self::build_minimal_yaml_for_context(name, &contexts, &clusters, &users)? {
  ...
  fn build_minimal_yaml_for_context(
    ...
    for nc in contexts.iter() {
      if nc.name == name {
        let cluster_entry: Option<KCNamedCluster> = clusters.iter().find(|c| c.name == nc.context.cluster).cloned();
        let user_entry: Option<KCNamedUser> = nc.context.user.as_ref().and_then(|uname| users.iter().find(|u| u.name == *uname).cloned());

        let out_ctx: OutNamedContext = OutNamedContext { name: name.to_string(), context: nc.context.clone() };
