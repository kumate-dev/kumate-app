...
impl K8sReplicationControllers {
    pub async fn list(
        ...
        let mut out: Vec<ReplicationControllerItem> =
            rcs.into_iter().map(Self::rc_to_item).collect();
        out.sort_by(
    ...
    fn rc_to_item(r: ReplicationController) -> ReplicationControllerItem {
        let replicas: i32 = r
            .spec
            .as_ref()
            .and_then(|sp: &ReplicationControllerSpec| sp.replicas)
            .unwrap_or(0);
        let ready: i32 = r
            .status
            .as_ref()
            .and_then(|st: &ReplicationControllerStatus| st.ready_replicas)
            .unwrap_or(0);
