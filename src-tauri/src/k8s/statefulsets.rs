...
impl K8sStatefulSets {
    pub async fn list(name: String, namespace: Option<String>) -> Result<Vec<StatefulSetItem>, String> {
    ...
    async fn fetch(client: Client, namespace: Option<String>) -> Result<Vec<StatefulSet>, String> {
        ...
        let list: ObjectList<StatefulSet> = api.list(&lp).await.map_err(|e: kube::Error| e.to_string())?;
        Ok(list.items)
    }

    fn sset_to_item(s: StatefulSet) -> StatefulSetItem {
        let replicas: i32 = s.spec.as_ref().and_then(|sp: &StatefulSetSpec| sp.replicas).unwrap_or(0);
        let ready: i32 = s.status.as_ref().and_then(|st: &StatefulSetStatus| st.ready_replicas).unwrap_or(0);
        StatefulSetItem {
            name: s.name_any(),
            namespace: s.namespace().unwrap_or_else(|| "default".to_string()),
            ready: format!("{}/{}", ready, replicas),
            creation_timestamp: s.metadata.creation_timestamp.map(|t: Time| t.0.to_rfc3339()),
        }
    }
}