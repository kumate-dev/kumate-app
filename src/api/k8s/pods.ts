...
export interface LogEvent {
  type: 'LOG_LINE' | 'LOG_LINES' | 'LOG_ERROR' | 'LOG_COMPLETED';
  pod: string;
  namespace: string;
  container?: string;
  /** Present on the legacy single-line `LOG_LINE` shape. */
  log?: string;
  /** Present on `LOG_LINES`: a coalesced batch of lines (see `watch_logs`). */
  logs?: string[];
  error?: string;
  timestamp: string;
}
