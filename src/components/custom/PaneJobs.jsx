...
import { listJobs, listNamespaces } from "../../services/k8s";
import { relativeAge } from "../../utils/time";
import { progressVariant } from "../../utils/k8s";
import { useNamespaceStore } from "../../state/namespaceStore";



export default function PaneJobs({ context }) {
  ...
  useEffect(() => {
    ...
    async function fetch() {
      ...
      try {
        ...
        const res = await withTimeout(listJobs({ name: context.name, namespace: nsParam }), 15000);
        ...
        <Table>
          ...
          <Tbody>
            ...
            {!loading && filtered.length === 0 && (
              <Tr><Td colSpan={5} className="text-white/60">No jobs</Td></Tr>
            )}
            {!loading && filtered.map((d) => (
              <Tr key={d.name}>
                <Td className="font-medium">{d.name}</Td>
                <Td className="text-white/80">{d.namespace}</Td>
                <Td><Badge variant={progressVariant(d.progress)}>{d.progress}</Badge></Td>
