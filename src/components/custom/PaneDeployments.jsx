...
import { listDeployments, listNamespaces } from "../../services/k8s";
import { relativeAge } from "../../utils/time";
import { readyVariant, deploymentStatusVariant } from "../../utils/k8s";
import { useNamespaceStore } from "../../state/namespaceStore";


export default function PaneDeployments({ context }) {
  ...
  useEffect(() => {
    ...
    async function fetchNamespaces() {
      ...
      // Reuse cached namespaces if context matches
  ...
  useEffect(() => {
    ...
    async function fetch() {
      ...
      try {
        ...
        const res = await withTimeout(listDeployments({ name: context.name, namespace: nsParam }), 15000);
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-white/10 bg-neutral-900/60 overflow-hidden">
        <Table>
          ...
          <Tbody>
            {loading && (
              <Tr><Td colSpan={6} className="text-white/60">Loading...</Td></Tr>
            )}
            {!loading && filtered.length === 0 && (
              <Tr><Td colSpan={6} className="text-white/60">No deployments</Td></Tr>
            )}
