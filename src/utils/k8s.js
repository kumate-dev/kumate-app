export function readyVariant(ready) {
  try {
    const [a, b] = String(ready).split("/").map((x) => parseInt(x, 10));
...
export function progressVariant(progress) {
  try {
    const [succ, comp] = String(progress).split("/").map((x) => parseInt(x, 10));
...
export function podStatusVariant(s) {
...
export function deploymentStatusVariant(s) {
