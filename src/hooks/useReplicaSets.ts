...
export function useReplicaSets(context?: K8sContext | null, namespace?: string) {
  const [items, setItems] = useState<ReplicaSet[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchReplicaSets() {
      ...
      try {
        ...
        const res = await withTimeout(listReplicaSets({ name, namespace: nsParam }), 15000);
